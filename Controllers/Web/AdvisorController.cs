...
namespace BilgiYonetimSistemi.Controllers.Web
{
    public class AdvisorController : Controller
    {
        ...
        public async Task<IActionResult> AdvisorPanel()
        {
            ...
            {
                ViewBag.PendingCourses = null;
                TempData["ErrorMessage"] = "Ders listesi yüklenirken bir hata oluştu.";
            }
        ...
        public async Task<IActionResult> UpdateAdvisor(int id, string newEmail, string newPassword)
        {
            ...
            if (!string.IsNullOrEmpty(newEmail))
            {
                ...
                var advisorJsonContent = new StringContent(JsonConvert.SerializeObject(updatedAdvisor), Encoding.UTF8, "application/json");
            ...
            if (!string.IsNullOrEmpty(userIdString))
            {
                ...
                var updatedUser = new
                {
                    ...
                    PasswordHash = !string.IsNullOrEmpty(newPassword) ? newPassword : user.passwordHash,
                    Email = !string.IsNullOrEmpty(newEmail) ? newEmail : user.email,
                    Role = user.role,
                    RelatedID = user.relatedID
                };

                var userJsonContent = new StringContent(JsonConvert.SerializeObject(updatedUser), Encoding.UTF8, "application/json");
